...
1. **Buscar Vimeo (Scraper Avanzado)**:
   ...
   * **Parche Antibloqueo (API macOS 401)**: El servidor intercepta los enlaces de Vimeo privados (tanto los que usan un hash en la URL como los protegidos por contraseña) y los transforma dinámicamente al formato de reproductor (`player.vimeo.com`) antes de enviarlos a yt-dlp. Esto puentea de manera definitiva el error *401 Unauthorized* causado por la reciente rotación de claves del cliente de macOS de Vimeo.
